...
// ==================== AJUSTES DE PAGO (TARJETA + FACTURA) ====================

document.querySelectorAll('.select-tipo-pago').forEach(select => {
    const ticketId = select.dataset.ticketId;
    const facturaCheck = document.getElementById(`facturaCheck${ticketId}`);

    select.addEventListener('change', () => recalcularTotal(ticketId));
    if (facturaCheck) {
        facturaCheck.addEventListener('change', () => recalcularTotal(ticketId));
    }

    recalcularTotal(ticketId);
});
...
function recalcularTotal(ticketId) {
    ...
    const select = document.querySelector(`.select-tipo-pago[data-ticket-id="${ticketId}"]`);
    const metodo = select.options[select.selectedIndex].dataset.metodo;

    const facturaCheck = document.getElementById(`facturaCheck${ticketId}`);
    const badgeTarjeta = document.getElementById(`badgeTarjeta${ticketId}`);
    const badgeFactura = document.getElementById(`badgeFactura${ticketId}`);
    const totalFinalInput = document.getElementById(`totalFinal${ticketId}`);
    const requiereFacturaInput = document.getElementById(`requiereFactura${ticketId}`);

    let porcentaje = 0;

    if (metodo === 'tarjeta') {
        porcentaje += 4;
        badgeTarjeta.classList.remove('d-none');
    } else {
        badgeTarjeta.classList.add('d-none');
    }

    if (facturaCheck && facturaCheck.checked) {
        porcentaje += 16;
        badgeFactura.classList.remove('d-none');
        requiereFacturaInput.value = 1;
    } else {
        badgeFactura.classList.add('d-none');
        requiereFacturaInput.value = 0;
    }

    const total = totalBase + (totalBase * porcentaje / 100);
    ...
    const montoInput = document.getElementById(`monto_recibido_${ticketId}`);
    if (montoInput) {
        montoInput.dataset.total = total.toFixed(2);
    }
